...
public class GestorFechas {
    ...
    public void seleccionarFechas(int offset, int diasEstancia) {
        ...
        while (intentos < 3 && !encontrado) {
            try {
                ...
                List<DiaCalendario> calendario = diasVisibles.stream()
                        ...
                        .filter(d -> !d.fecha.isBefore(fechaReferencia))
                        .toList();
                ...
                if (!encontrado) {
                    ...
                    // Actualizar la fecha de referencia para el siguiente intento
                    if (!calendario.isEmpty()) {
                        fechaReferencia = calendario.get(0).fecha.plusDays(1);
                        LOGGER.info("Buscando siguiente bloque disponible después de: " + fechaReferencia);
                    }
                }
            } catch (Exception e) {
        ...
        if (!encontrado) {
            LOGGER.error("No se encontraron fechas disponibles después de 3 intentos.");
            throw new IllegalStateException("No se encontraron fechas disponibles después de 3 intentos.");
        }
